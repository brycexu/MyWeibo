			...
			<c:forEach items="${commentList }" var="comment" varStatus="status">
				...
				<div class="container" style="width: 850px; background-color: white;">
					...
					<div
						...
						<c:if test="${comment.user.userId==user.userId}">
							<img onclick="javascript:clickme();"
								src="/imgUpload/${comment.user.face}" width="50px" height="50px"
								class="img-circle">
						</c:if>
						<c:if test="${comment.user.userId!=user.userId}">
					...
					<div style="text-align: left; margin: 10px; float: left;">
						<!-- 昵称 -->
						<a style="color: #333; font-size: 15px" href="javascrip:;">${comment.user.nickname }</a>
						<!-- 时间 -->
						<p style="color: #999; font-size: 15px">${comment.time }</p>
					</div>
				</div>
				<div class="container" style="width: 850px; background-color: #fff; margin-bottom: 10px;">
					...
					<div style="background-color: #eaeaec;float: left;margin-left: 85px; margin-bottom: 10px;">
						...
						<a style="color: #333; font-size: 15px" href="singleWeibo.action?weiboId=${ comment.weibo.weiboId}">${comment.weibo.content }</a>
					</div>
				</div>
				<br>
			</c:forEach>
