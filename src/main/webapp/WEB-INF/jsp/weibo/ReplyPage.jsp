		...
		<h2 style="color: #333; margin:1px;">收到的回复<span style="color: #f00;" class="glyphicon glyphicon-comment"></span></h2>
			...
			<c:forEach items="${replyList }" var="reply" varStatus="status">
				...
				<div class="container" style="width: 850px; background-color: white;">
					...
					<div
						...
						<c:if test="${reply.fromId==user.userId}">
							<img onclick="javascript:clickme();"
								src="/imgUpload/${reply.from_face}" width="50px" height="50px"
								class="img-circle">
						</c:if>
						<c:if test="${reply.fromId!=user.userId}">
							<img onclick="javascript:clickother(${comment.user.userId});"
								src="/imgUpload/${reply.from_face}" width="50px" height="50px"
								class="img-circle">
						</c:if>
					</div>
					<div style="text-align: left; margin: 10px; float: left;">
						<!-- 昵称 -->
						<a style="color: #333; font-size: 15px" href="javascrip:;">${reply.from }</a>
						<!-- 时间 -->
						<p style="color: #999; font-size: 15px">${reply.rtime }</p>
					</div>
				</div>
				<div class="container" style="width: 850px; background-color: #fff; margin-bottom: 10px;">
					...
					<div style="text-align: left; margin-left: 85px" >
						...
						<a href="#" style="float: left;">@${reply.to }</a>
					...
					<div style="background-color: #eaeaec;float: left;margin-left: 85px; margin-bottom: 10px;">
						...
						<a style="color: #333; font-size: 15px" href="singleWeibo.action?weiboId=${ reply.weibo.weiboId}">${reply.weibo.content }</a>
					</div>
				</div>
				<br>
			</c:forEach>
			<div class="container" style="width: 850px;">
				...
				<ul class="pagination pagination-lg">
					<!-- 上一页 -->
					<li><a href="toCommentPage.action?pageNo=${page.pageNo-1 }">&laquo;</a></li>
					<c:choose>
					...
					<c:forEach begin="${begin}" end="${end}" var="i">
						...
						<c:if test="${i!=page.pageNo }">
							<li><a href="toCommentPage.action?pageNo=${i }">${i}</a></li>
						</c:if>
					</c:forEach>
					<!-- 下一页 -->
					<li><a href="toCommentPage.action?pageNo=${page.pageNo+1 }">&raquo;</a></li>
				</ul>
			</div>
